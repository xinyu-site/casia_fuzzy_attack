...
class StarCraft2Env(MultiAgentEnv):
    ...
    def get_obs_agent(self, agent_id, fully_observable=False):
        ...
        _, enemy_feats_dim = self.get_obs_enemy_feats_size()
        _, ally_feats_dim = self.get_obs_ally_feats_size()
        own_feats_dim = self.get_obs_own_feats_size()

        # move_feats = np.zeros(move_feats_dim, dtype=np.float32)    # 移动方向4
        enemy_feats = np.zeros((self.n_enemies, enemy_feats_dim - 2), dtype=np.float32)  # 敌人特征[n_enemy, 9]
        ally_feats = np.zeros((self.n_agents - 1, ally_feats_dim - 2), dtype=np.float32)    # 队友特征[n_agent-1, 9]
        own_feats = np.zeros(own_feats_dim - 2, dtype=np.float32)      # 自身特征7

        enemy_equ_feats = np.zeros((self.n_enemies, 2), dtype=np.float32)
        ally_equ_feats = np.zeros((self.n_agents - 1, 2), dtype=np.float32)
        own_equ_feats = np.zeros(2, dtype=np.float32)
        ...
        if (
            ...
            for e_id, e_unit in self.enemies.items():
                ...
                if (enemy_visible and e_unit.health > 0) or (
                    ...
                    enemy_equ_feats[e_id, 0] = (
                        e_x - x
                    ) / sight_range  # relative X
                    enemy_equ_feats[e_id, 1] = (
                        e_y - y
                ...
                if self.obs_enemies[e_id, agent_id] == 0:
                    # enemy_feats = np.zeros((self.n_enemies, enemy_feats_dim - 2), dtype=np.float32)
                    enemy_feats[e_id] = np.zeros(enemy_feats_dim - 2, dtype=np.float32)
                    enemy_equ_feats[e_id] = np.zeros(2, dtype=np.float32)
            # Ally features
            al_ids = [
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if (ally_visible and al_unit.health > 0) or (
                    ...
                    ally_equ_feats[i, 0] = (al_x - x) / sight_range  # relative X
                    ally_equ_feats[i, 1] = (al_y - y) / sight_range  # relative Y
            ...
            if self.stochastic_health:
                ...
                ind += 1
            if self.obs_own_pos:
                own_equ_feats[0] = x / self.map_x
                own_equ_feats[1] = y / self.map_y
                # ind += 2
            if self.conic_fov:
        ...
        if self.obs_starcraft:
            agent_obs = np.concatenate(
                (
                    enemy_equ_feats.flatten(),
                    ally_equ_feats.flatten(),
                    own_equ_feats.flatten(),
    ...
    def get_state(self):
        ...
        agents = list(range(self.n_agents))
        random.shuffle(agents)
        agents_state = [None for i in range(len(agents))]
        for agent in agents:
            agents_state[agent] = self.get_state_agent(
                agent
            )
        
        state = np.concatenate(agents_state, axis=0).astype(np.float32)
        # if self.debug:
        # state_dict = self.get_state_dict()
        ...
        #     state_dict["allies"].flatten(), state_dict["enemies"].flatten()
        # )
        # state = state_dict["allies"].flatten()
        # if "last_action" in state_dict:
        #     state = np.append(state, state_dict["last_action"].flatten())
        # if "timestep" in state_dict:
        #     state = np.append(state, state_dict["timestep"])
    ...
    def get_state_dict(self):
        ...
        nf_al = self.get_ally_num_attributes()  # 8维
        nf_en = self.get_enemy_num_attributes() # 7维
    ...
    def get_state_agent(self, agent_id):
        ...
        ally_state = np.zeros((self.n_agents - 1, nf_al - 2))
        ally_equ_state = np.zeros((self.n_agents - 1, 2))

        enemy_state = np.zeros((self.n_enemies, nf_en - 2))
        enemy_equ_state = np.zeros((self.n_enemies, 2))
        
        own_state = np.zeros(nf_al - 2)
        own_equ_state = np.zeros(2)
        ...
        if unit.health > 0:
            ...
            for i, al_id in enumerate(al_ids):
                ...
                if al_unit.health > 0:
                    ...
                    if not self.stochastic_health:
                        ...
                        ally_state[i, 0] = self._compute_health(al_id, al_unit)
                    # 冷却时间
                    if (
                        self.map_type in ["MMM", "terran_gen"]
                        and al_unit.unit_type == self.medivac_id
                    ):
                        ally_state[i, 1] = al_unit.energy / max_cd  # energy
                    else:
                        ally_state[i, 1] = (
                            al_unit.weapon_cooldown / max_cd
                        )  # cooldown
                    ...
                    if self.stochastic_attack:
                        ally_state[i, ind] = self.agent_attack_probabilities[
                            al_id
                        ]
                        ind += 1
                    if self.stochastic_health:
                        ally_state[i, ind] = self.agent_health_levels[al_id]
                        ind += 1
                    if self.conic_fov:
                        ally_state[i, ind : ind + 2] = self.fov_directions[
                            al_id
                        ]
                    # 单位类型
                    if self.unit_type_bits > 0:
                        type_id = self.get_unit_type_id(al_unit, True)
                        ally_state[i, type_id - self.unit_type_bits] = 1
            # 敌人状态
            for e_id, e_unit in self.enemies.items():
                if e_unit.health > 0:
                    e_x = e_unit.pos.x
                    ...
                    # 等变特征
                    # enemy_equ_state[e_id, 0] = (
                    #     e_x - center_x
                    # ) / self.max_distance_x  # relative X
                    # enemy_equ_state[e_id, 1] = (
                    #     e_y - center_y
                    # ) / self.max_distance_y  # relative Y
                    enemy_equ_state[e_id, 0] = (
                        e_x - x
                    ...
                    if self.shield_bits_enemy > 0:
                        max_shield = self.unit_max_shield(e_unit)
                        enemy_state[e_id, 2] = e_unit.shield / max_shield  # shield
                    # 类型
                    if self.unit_type_bits > 0:
                        type_id = self.get_unit_type_id(e_unit, False)
                        enemy_state[e_id, type_id - self.unit_type_bits] = 1
            ...
            # 自身状态
            own_equ_state[0] = x / self.max_distance_x
            own_equ_state[1] = y / self.max_distance_y
            ind = 0
            # 血量
            if not self.stochastic_health:
                own_state[ind] = unit.health / unit.health_max
            else:
                own_state[ind] = self._compute_health(agent_id, unit)
            ind += 1
            # 冷却时间
            ...
            if (
                ...
                own_state[ind] = al_unit.energy / max_cd  # energy
            else:
                own_state[ind] = (
                    al_unit.weapon_cooldown / max_cd
                )  # cooldown
            ...
            if self.shield_bits_ally > 0:
                max_shield = self.unit_max_shield(unit)
                own_state[ind] = unit.shield / max_shield
                ind += 1
            if self.stochastic_attack:
                own_state[ind] = self.agent_attack_probabilities[agent_id]
                ind += 1
            if self.stochastic_health:
                own_state[ind] = self.agent_health_levels[agent_id]
                ind += 1
            if self.conic_fov:
                own_state[ind : ind + 2] = self.fov_directions[agent_id]
                ind += 2
            # 类型
            if self.unit_type_bits > 0:
                type_id = self.get_unit_type_id(unit, True)
                own_state[ind + type_id] = 1
        
        state = np.concatenate(
            (
                enemy_equ_state.flatten(),
                ally_equ_state.flatten(),
                own_equ_state.flatten(),
    ...
    #     ally_state = np.zeros((self.n_agents, nf_al))
    #     # equ_state = np.zeros()
    #     all_enemy_state = np.zeros((self.n_agents, self.n_enemies * (nf_en + 1)))
    ...
    #             # 位置
    #             ally_state[al_id, 0] = x / self.max_distance_x  # X
    #             ally_state[al_id, 1] = y / self.max_distance_y  # relative Y
    ...
    #                 )  # health
    #             else:
    #                 ally_state[al_id, 2] = self._compute_health(al_id, al_unit)
    ...
    #             # 敌方观测
    #             enemy_state = np.zeros((self.n_enemies, nf_en + 1))
    ...
    #                     enemy_state[e_id, 0] = np.linalg.norm(
    #                         [x - e_x, y - e_y]
    #                     ) / np.linalg.norm([self.max_distance_x, self.max_distance_y]) # dis

    #                     theta = np.arctan2(e_y - y, e_x - x) - al_unit.facing

    #                     enemy_state[e_id, 1] = np.cos(theta)
    #                     enemy_state[e_id, 2] = np.sin(theta)
    ...
    #                     if self.shield_bits_enemy > 0:
    #                         max_shield = self.unit_max_shield(e_unit)
    #                         enemy_state[e_id, 4] = e_unit.shield / max_shield  # shield

    #                     if self.unit_type_bits > 0:
    #                         type_id = self.get_unit_type_id(e_unit, False)
    #                         enemy_state[e_id, type_id - self.unit_type_bits] = 1
    #             enemy_state = enemy_state.reshape(-1,)
    #             all_enemy_state[al_id] = enemy_state.copy()

    #     # state = {"allies": ally_state, "enemies": enemy_state}
    #     state = {"allies": np.concatenate([ally_state, all_enemy_state], axis=-1)}
    ...
    def get_state_size(self):
        ...
        size = self.n_agents * (self.n_agents * nf_al + self.n_enemies * nf_en)
