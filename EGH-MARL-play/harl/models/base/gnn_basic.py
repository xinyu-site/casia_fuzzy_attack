...
class InvariantScalarNet(nn.Module):
    ...
    def forward(self, vectors, scalars=None):
        ...
        """
        if type(vectors) == list:
            Z = torch.stack(vectors, dim=-1)  # [N, 3, K] K the length of the list
        else:
            Z = vectors
        K = Z.shape[-1]
        Z_T = Z.transpose(-1, -2)  # [N, K, 3]
...
class EGNN_Layer(nn.Module):
    ...
    def forward(self, x, h, edge_index, edge_fea, v=None):
        row, col = edge_index
        rij = x[row] - x[col]  # [BM, 3], M is the number of edges. rij is xij
        hij = torch.cat((h[row], h[col], edge_fea), dim=-1)  # [BM, 2K+T], hij is the concat of hi, hj and edge feature
        message = self.edge_message_net(vectors=[rij], scalars=hij)  # [BM, K], K is the output_dim, message is mij
        coord_message = self.coord_net(message)  # [BM, 1]
        f = (x[row] - x[col]) * coord_message  # [BM, 3] update of mij
        ...
        tot_message = aggregate(message=message, row_index=row, n_node=x.shape[0], aggr='sum')  # [BN, K], sum of mij
        node_message = torch.cat((h, tot_message), dim=-1)  # [BN, K+K]
        h = self.node_net(node_message)  # [BN, K], update of h
        return x, v, h
