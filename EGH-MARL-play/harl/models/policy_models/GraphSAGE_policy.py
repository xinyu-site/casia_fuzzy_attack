...
class GraphSAGEPolicy(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        """
        super(GraphSAGEPolicy, self).__init__()
        ...
        self.local_tool = local_tool(self, args=args, net_type="graphsage", id="actor")
        ...
        if self.local_mode:
            ...
            for i in range(len(self.local_info_output)):
                ...
                # layer = SAGEConv(self.local_info_input[i], self.local_info_output[i])
        ...
        for i in range(0, self.n_layers):
            self.add_module("actor_graphsage_%d" % i, SAGEConv(int(self.hidden_sizes[0]), int(self.hidden_sizes[0])))    
        self.actor_fc1 = nn.Linear(self.hidden_sizes[0], act_dim)
    ...
    def evaluate_actions(
        ...
        for j in range(0, self.n_layers):
            ...
            x = self._modules["actor_graphsage_%d" % j](x, self.local_tool.eval_edges)
            # 临时修改，用于评估
            # x = self._modules["actor_graphsage_%d" % j](x, self.local_tool.forward_edges)
