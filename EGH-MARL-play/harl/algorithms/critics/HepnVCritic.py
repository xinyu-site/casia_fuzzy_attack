...
from harl.models.value_function_models.hepn_critic_net import HepnCriticNet
from harl.algorithms.critics.v_critic import VCritic


class HepnVCritic(VCritic):
    """V Critic.
    Critic that learns a V-function.
    """
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        super(HepnVCritic, self).__init__(args, cent_obs_space, device)
        self.lp_coef = args["critic_lp_coef"]
        self.critic = HepnCriticNet(args, self.share_obs_space, self.device)
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks, layer_data):
        ...
        """
        values, rnn_states_critic = self.critic(cent_obs, rnn_states_critic, masks, layer_data)
    ...
    def update(self, sample, value_normalizer=None):
        ...
        values, _ = self.get_values(
            share_obs_batch, rnn_states_critic_batch, masks_batch, layer_data
        )
