...
def make_train_env(env_name, seed, n_threads, env_args):
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
            ...
            elif env_name == "mujoco3d":
                from harl.envs.mujoco3d.mujoco3d_env import Mujoco3D
                env = Mujoco3D(env_args=env_args)
                ...
                from harl.envs.ma_envs.rendezvous_env import RENDEEnv
                env = RENDEEnv(env_args)
            elif env_name == "pursuit":
                from harl.envs.ma_envs.pursuit_env import PursuitEnv
                ...
                from harl.envs.ma_envs.navigation_env_v2 import NavigationV2Env
            ...
            #     from harl.envs.overcooked_ai.overcooked_ai_env import OvercookedAIEnv
            #     env = OvercookedAIEnv(env_args)
            else:
    ...
    if n_threads == 1:
        if env_args["structural_entropy"]:
            return ShareDummyVecEnv([get_env_fn(0)], hie=True)
        else:
            return ShareDummyVecEnv([get_env_fn(0)])
    else:
        if env_args["structural_entropy"]:
            return ShareSubprocVecEnv([get_env_fn(i) for i in range(n_threads)], hie=True)
        else:
