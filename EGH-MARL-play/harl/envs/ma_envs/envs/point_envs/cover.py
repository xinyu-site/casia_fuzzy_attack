...
from harl.envs.ma_envs.agents.point_agents.cover_agent import CoverAgent
...
class CoverEnv_ori(gym.Env, EzPickle):
    ...
    def __init__(self,
        ...
        self.cover_range = 5
        self.total_cover_num_per_algo_episode = 0
        self.safe_dis = 9.0
        if self.obs_mode == 'sum_obs_learn_comm':
            self.world.dim_c = 1
        # self.filename = f'coverage_index_{os.getpid()}.csv'
        # with open(self.filename, mode='w', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(['Time Step', 'Coverage Index'])  # 可以在这里写入头部
        # self.seed()
    ...
    def reset(self):
        ...
        if self.obs_radius2 > self.world_size * np.sqrt(2):
    ...
    # def write_coverage_index_to_csv(self, coverage_index):
    #     with open(self.filename, mode='a', newline='') as file:
    #         writer = csv.writer(file)
    #         writer.writerow([self.timestep, coverage_index])
    ...
    def step(self, actions):
        ...
        rewards , cover_reward , energy_penalty , gather_penalty = self.get_reward(actions)
        ...
        # return next_obs, s_obs, rewards, cover_reward, energy_penalty, gather_penalty, done, info, None
    ...
    def get_reward(self, actions):
        ...
        r1 = np.zeros((self.nr_agents, 1))
        r2 = np.zeros((self.nr_agents, 1))
        r3 = np.zeros((self.nr_agents, 1))
        ...
        for i in range(self.nr_agents):
            ...
            r1[i] += cover_reward * cover_ratio / self.nr_agents
            ...
            r2[i] += energy_penalty_rate * energy_consumed
