...
class Evader(Agent):
    ...
    def step_apf(self, agent, world):
        Kr = 0.5  # 追逐者和墙壁的斥力系数
        epsilon = 1e-6  # 避免除以零
        agent_do = 30.0  # 斥力作用的距离阈值
        wall_do = 30.0
        evader_cor = self.state.p_pos  # 逃跑者位置
        world_size = world.world_size  # 地图尺寸

        # 初始化总斥力向量
        total_rep_force = np.zeros(2)

        # 计算追逐者产生的斥力
        for pursuer_cor in world.agent_states[:, 0:2]:
            distance = np.linalg.norm(evader_cor - pursuer_cor) + epsilon
            if distance < agent_do:
                rep_force = Kr * (1 / distance - 1 / agent_do) * (evader_cor - pursuer_cor) / distance ** 3
                total_rep_force += rep_force

        # 计算墙壁产生的斥力
        boundaries = [evader_cor[0], world_size - evader_cor[0], evader_cor[1], world_size - evader_cor[1]]
        for i, distance in enumerate(boundaries):
            distance = distance + epsilon
            if distance < wall_do:
                rep_force_direction = np.array([0, 0])
                if i == 0:
                    rep_force_direction[0] = 1  # 左墙
                elif i == 1:
                    rep_force_direction[0] = -1  # 右墙
                elif i == 2:
                    rep_force_direction[1] = 1  # 下墙
                elif i == 3:
                    rep_force_direction[1] = -1  # 上墙
                rep_force = Kr * (1 / distance - 1 / wall_do) * rep_force_direction / distance ** 3
                total_rep_force += rep_force

        # 如果斥力非零，计算逃跑智能体的速度
        if np.linalg.norm(total_rep_force) > epsilon:
            velocity = total_rep_force / np.linalg.norm(total_rep_force)
        else:
            # 计算与所有追逐者的平均位置向量
            average_pursuer_vector = np.mean(world.agent_states[:, 0:2] - evader_cor, axis=0)
            if np.linalg.norm(average_pursuer_vector) > epsilon:
                # 选择远离平均位置的方向作为逃跑方向
                velocity = -average_pursuer_vector / np.linalg.norm(average_pursuer_vector)
            else:
                # 如果所有追逐者平均位置非常接近逃跑者（极不可能），随机选择一个方向
