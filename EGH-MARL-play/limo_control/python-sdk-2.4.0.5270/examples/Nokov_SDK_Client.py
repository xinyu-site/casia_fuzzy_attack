...
def main(argv):
    ...
    print('NokovSDK Sample Client 2.4.0.5270(NokovSDK ver. %d.%d.%d.%d)' % (ver[0], ver[1], ver[2], ver[3]))
    ...
    for iDef in range(dataDefs.nDataDescriptions):
        ...
        if dataDef.type is Descriptor_Skeleton:
            skeletonDef = dataDef.Data.SkeletonDescription.contents
            print("Skeleton Name:%s" % skeletonDef.szName)
            for iBody in range(skeletonDef.nRigidBodies):
                bodyDef = skeletonDef.RigidBodies[iBody]
                print("\tRigidBody %s %d:Tx:%lf Ty:%lf Tz:%lf qx:%lf qy:%lf qz:%lf qw:%lf\n" %
                (bodyDef.szName,
                bodyDef.ID,
                bodyDef.offsetx,
                bodyDef.offsetx,
