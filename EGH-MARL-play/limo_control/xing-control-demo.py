    ...
    if pFrameOfMocapData == None:  
        ...
        for iMarkerSet in range(frameData.nMarkerSets):
            ...
            if vel_name[4:] in robot_list:
                ...
                if vel_name[4:] in move_target_list:
                    vel_index = move_target_list.index(vel_name[4:]) + num_agents
                    obj_type = 1
            ...
            if obj_type == 1 or obj_type == 2:   
                ...
                # method = CalculateVelocity(90, 19) # FPS:60 FrameFactor:3 which means the first 2 frame has None
        ...
        if (frame_cnt % 100) == 0:
            ...
            if scenario == "pursuit":
                ...
                prey_action = np.stack(prey_action)
                prey_action = move_action_adjust(prey_action, back_raw[num_agents:])
                mover_output_action = prey_action
            else:
