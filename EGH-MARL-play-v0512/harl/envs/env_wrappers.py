...
def shareworker(remote, parent_remote, env_fn_wrapper, hie):
    parent_remote.close()
    ...
    while True:
        ...
        if cmd == "step":
            if hie:
                ob, s_ob, layer_data, reward, done, info, available_actions = env.step(data)
            else:
            ...
            if "bool" in done.__class__.__name__:  # done is a bool
                if (
                    ...
                    if hie:
                        ob, s_ob, layer_data, available_actions = env.reset()
                    else:
                ...
                if np.all(
                    ...
                    if hie:
                        ...
                        ob, s_ob, available_actions = env.reset()
            if hie:
                remote.send((ob, s_ob, layer_data, reward, done, info, available_actions))
            else:
                remote.send((ob, s_ob, reward, done, info, available_actions))
        elif cmd == "reset":
            if hie:
                ob, s_ob, layer_data, available_actions = env.reset()
                remote.send((ob, s_ob, layer_data, available_actions))
            else:
                ob, s_ob, available_actions = env.reset()
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None, hie=False):
        """
        ...
        self.ps = [
            Process(
                target=shareworker,
                args=(work_remote, remote, CloudpickleWrapper(env_fn), self.hie),
            )
    ...
    def step_wait(self):
        ...
        self.waiting = False
        if self.hie:
            obs, share_obs, layer_data, rews, dones, infos, available_actions = zip(*results)
            return (
                np.stack(obs),
                np.stack(share_obs),
                np.stack(layer_data),
    ...
    def reset(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        if self.hie:
            obs, share_obs, layer_data, available_actions = zip(*results)
            return np.stack(obs), np.stack(share_obs), np.stack(layer_data), np.stack(available_actions)
        else:
...
class ShareDummyVecEnv(ShareVecEnv):
    def __init__(self, env_fns, hie=False):
    ...
    def step_wait(self):
        ...
        for i, done in enumerate(dones):
            if "bool" in done.__class__.__name__:  # done is a bool
                if (
                    ...
                    if self.hie:
                        obs[i], share_obs[i], layer_data[i], available_actions[i] = self.envs[i].reset()
                    else:
        ...
        if self.hie:
            return obs, share_obs, layer_data, rews, dones, infos, available_actions
        else:
