...
class SMACLogger(BaseLogger):
    def __init__(self, args, algo_args, env_args, num_agents, writter, run_dir):
        ...
        self.win_rate_file = open(
            os.path.join(run_dir, "win_rate.txt"), "w", encoding="utf-8"
        )
        self.win_key = "won"
    ...
    def per_step_offpolicy(self, data):
        (
            share_obs,
            obs,
            actions,
    ...
    def episode_log(
        ...
        self.writter.add_scalars(
            ...
            {"average episode reward": average_episode_len * critic_train_info["average_step_rewards"]},
            self.total_num_steps,
        )
        self.train_log_file.write(
            ",".join(map(str, [self.total_num_steps, average_episode_len * critic_train_info["average_step_rewards"]])) + "\n"
        )
        self.train_log_file.flush()

        self.win_rate_file.write(
            ",".join(map(str, [self.total_num_steps, incre_win_rate])) + "\n"
        )
        self.win_rate_file.flush()
    ...
    def episode_log_offpolicy(self):
        self.total_num_steps = (
            self.episode
            * self.algo_args["train"]["episode_length"]
            * self.algo_args["train"]["n_rollout_threads"]
        ...
        print(
            "Increase games {:.4f}, win rate on these games is {:.4f}, average episode length is {:.4f}.\n".format(
                np.sum(incre_battles_game),
                incre_win_rate,
                # critic_train_info["average_step_rewards"],
                average_episode_len,
    ...
    def eval_thread_done(self, tid):
        ...
        if self.win_key not in self.eval_infos[tid][0]:
            return
