...
class ModularEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, a):
        ...
        if "right_fthigh" in self.model.body_names:
            torso_height = min(torso_height, self.data.get_body_xpos("right_fthigh")[2])
        if "left_fthigh" in self.model.body_names:
            torso_height = min(torso_height, self.data.get_body_xpos("left_fthigh")[2])

        done = not (torso_height > 0.26 and abs(pitch) < 1.35 and abs(roll) < 1.35 and np.square(self.sim.data.qvel).sum() > 1)
        ob = self._get_obs()
