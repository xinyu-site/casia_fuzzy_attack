...
from harl.envs.mujoco3d.utils import *
import os
import math
...
class ModularEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        def _get_obs_per_limb(b):
            ...
            if b == "torso":
                ...
                axis_x = self.data.get_joint_xaxis(b+'_joint_x')  # 关节的x轴方向的单位向量
                axis_y = self.data.get_joint_xaxis(b+'_joint_y')  # 关节的y轴方向的单位向量
                axis_z = self.data.get_joint_xaxis(b+'_joint_z')  # 关节的z轴方向的单位向量
            
            obs = np.zeros(34 + len(limb_type_vec))
            pos = self.data.get_body_xpos(b)    # 位置
            dir = self.target - pos[:2]         # 目标方向
            dir = dir / np.linalg.norm(dir)     # 目标方向归一化
            dir = np.append(dir, 0)             # 目标方向补零，对齐维度
            vel = np.clip(self.data.get_body_xvelp(b), -10, 10)  # 速度
            wvel = self.data.get_body_xvelr(b)  # 角速度
            equ_obs = np.concatenate([pos, vel, wvel, dir, axis_x, axis_y, axis_z])

            gravity = np.array([0., 0., -9.81]) # 重力
            angles = np.array([
                np.cos(angle0_x), np.sin(angle0_x), 
                np.cos(angle0_y), np.sin(angle0_y), 
                np.cos(angle0_z), np.sin(angle0_z),
            ])
            inv_obs = np.concatenate([
                angles,
                gravity,
                limb_type_vec,    # 类型
                [self.data.get_body_xpos(b)[2]]  # 高度
            ])
            obs = np.concatenate([equ_obs, inv_obs])
            return obs
        ...
        full_obs = np.concatenate([full_obs, distances], axis=-1)
    ...
    #         obs[0:3] = xpos
    #         obs[3:5] = dir
    #         obs[6:9] = np.clip(self.data.get_body_xvelp(b), -10, 10)
    #         obs[9:12] = self.data.get_body_xvelr(b)
