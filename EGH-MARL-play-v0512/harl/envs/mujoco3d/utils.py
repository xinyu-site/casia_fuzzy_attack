...
import xmltodict
import harl.envs.mujoco3d.wrappers as wrappers
import gym
...
ENV_DIR = os.path.join(base_path, "environments")
XML_DIR = os.path.join(base_path, "environments/xmls")
BASE_MODULAR_ENV_PATH = os.path.join(base_path, "environments/ModularEnv.py")
...
def registerEnvs(env_name, max_episode_steps, custom_xml):
    ...
    # paths_to_register = []
    custom_xml = os.path.join(ENV_DIR, custom_xml)

    # custom envs
    assert os.path.isfile(custom_xml), "Custom xml file not found"
    xml = custom_xml
    ...
    register(
        ...
        entry_point="harl.envs.mujoco3d.environments.%s:ModularEnv" % env_file,
        kwargs=params,
    )
    env = wrappers.IdentityWrapper(gym.make("harl.envs.mujoco3d.environments:%s-v0" % env_name))
...
def getGraphStructure(xml_file, graph_type="morphology"):
    ...
    xml_file = os.path.join(ENV_DIR, xml_file)
