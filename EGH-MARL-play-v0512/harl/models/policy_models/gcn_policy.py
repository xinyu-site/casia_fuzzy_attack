...
class GCNPolicy(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self.local_tool = local_tool(self, args=args, net_type="gcn", id="actor")
        ...
        for i in range(0, self.n_layers):
            self.add_module("actor_gcn_%d" % i, GCNConv(int(self.hidden_sizes[0]), int(self.hidden_sizes[0])))    
    ...
    def forward(
        ...
        for j in range(0, self.n_layers):
            x_res = x  # 保留原始输入
            x = self._modules["actor_gcn_%d" % j](x, self.local_tool.forward_edges)
            if self.use_res:
                x = x + x_res  # 应用残差连接
            x = torch.tanh(x)
    ...
    def evaluate_actions(
        ...
        for j in range(0, self.n_layers):
            x_res = x  # 保留原始输入
            x = self._modules["actor_gcn_%d" % j](x, self.local_tool.eval_edges)
            if self.use_res:
                x = x + x_res  # 应用残差连接
            x = torch.tanh(x)
