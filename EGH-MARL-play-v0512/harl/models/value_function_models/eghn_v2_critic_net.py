...
class Eghnv2CriticNet(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        """
        super(Eghnv2CriticNet, self).__init__()
        self.args = args
        ...
        # self.collect_edges = self.get_edges_batch(self.n_threads)
        # # edges used for training
        # self.train_edges = self.get_edges_batch(self.mini_batch_size)
        ...
        self.critic_fc1 = nn.Linear(self.n_vector_input * self.n_vector_input + self.hidden_sizes[-1], 1) # 转置相乘+inv_equ
        # self.critic_fc1 = nn.Linear(self.hidden_sizes[-1], 1)  # 只使用不变性特征
        # self.critic_fc1 = nn.Linear(self.hidden_sizes[-1] + self.n_vector_input, 1) # 范数+inv_equ
        self.to(device)
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        if cent_obs.shape[0] == self.mini_batch_size:
            edges = self.local_tool.train_edges  # 用于训练的边
            batches = self.mini_batch_size
        else:
            edges = self.local_tool.collect_edges  # 用于收集数据的边
        ...
        equ_out, h_out = self.eghn_model(
            equ_fea, h, edges, None, local_edge_index, 
