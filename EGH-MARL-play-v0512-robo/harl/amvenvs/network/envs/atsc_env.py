...
class TrafficSimulator:
    def __init__(self, config, output_path, is_record, record_stats, port=0):
        ...
        self.data_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config', config.get('data_path'))
        ...
        for node_name in self.node_names:
            ...
            for lane_name in lanes_in:
                if self.name == 'atsc_real_net':
                    ...
                    cur_cap = 0
                    for ild_name in cur_ilds_in:                        cur_cap += self.sim.lane.getLength(ild_name)
