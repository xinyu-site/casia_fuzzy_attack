...
class CoverEnv:
    def __init__(self, args):
        self.args = args
        self.env = CoverEnv_ori(local_mode=self.args["local_mode"],
    ...
    def step(self, actions):
        # obs, s_obs, rewards, cover_reward, energy_penalty, gather_penalty, done, info, avaliable_actions = self.env.step(actions)
...
#                                 distance_thresold=self.args["distance_thresold"],
#                                     dim_mlp1_output=self.args["dim_mlp1_output"],
#                                     dim_mlp2_output=self.args["dim_mlp2_output"],
#                                     int_points_num=self.args["int_points_num"],
#                                     obs_mode=self.args["obs_mode"],
#                                     comm_radius=200 * np.sqrt(2),
#                                     # obs_radius=100 * np.sqrt(2),
#                                     world_size=self.args["world_size"],
#                                     distance_bins=self.args["distance_bins"],
#                                     bearing_bins=self.args["bearing_bins"],
#                                     torus=self.args["torus"],
