...
class CoverAgent(Agent):
    def __init__(self, experiment):
        ...
        if self.obs_mode == '2d_rbf':
            ...
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + np.prod(self.dim_int_points_o) + self.dim_equ_o
        elif self.obs_mode == 'global_obs_mode':
            ...
            self.dim_rec_o = (self.n_agents - 4, 3)
            self.dim_local_o = 3 * int(not self.torus)
            self.dim_equ_o = 4
            self.dim_int_points_o = (self.n_int_point - 20, 3)
            self.dim_flat_o = np.prod(self.dim_int_points_o) + self.dim_local_o
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + np.prod(self.dim_int_points_o) + self.dim_equ_o
        elif self.obs_mode == 'local_obs_mode':
            self.dim_rec_o = (self.n_agents - 1, 3)
            self.dim_local_o = 3 * int(not self.torus)
            self.dim_equ_o = 4
            self.dim_int_points_o = (self.n_int_point, 3)
            ...
            self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + np.prod(self.dim_int_points_o) + self.dim_equ_o + self.dim_adjacency_o
            ...
            self.dim_flat_o = np.prod(self.dim_int_points_o) + self.dim_local_o
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + np.prod(self.dim_int_points_o) + self.dim_local_o + self.dim_equ_o
            if self.local_mode:
    ...
    def get_observation(self, dm, my_orientation, their_orientation, feat, vels):
        ...
        if self.obs_mode == 'fix':
            ...
            int_points_obs = np.zeros(self.dim_int_points_o)
            int_points_obs[:, 0] = np.where(int_points_dists <= self.obs_radius, int_points_dists / self.obs_radius, 1)
            int_points_obs[:, 1] = np.where(int_points_dists <= self.obs_radius, np.cos(int_points_bearings), 0)
            int_points_obs[:, 2] = np.where(int_points_dists <= self.obs_radius, np.sin(int_points_bearings), 0)
            ...
            if self.local_mode:
                ...
                obs = np.hstack([rec_obs.flatten(), int_points_obs.flatten(), local_obs.flatten(), equ_obs.flatten(), adjacency_vector])
            else:
                obs = np.hstack([rec_obs.flatten(), int_points_obs.flatten(), local_obs.flatten(), equ_obs.flatten()])

        # 下边的观测模式是用来给mappo,rnn,data_aug用的
        elif self.obs_mode == 'temp_obs_mode':
            if self.obs_radius2 > 100:
                dist_to_int_points = int_points_dists / self.obs_radius2
                angle_to_int_points = [np.cos(int_points_bearings), np.sin(int_points_bearings)]
            ...
            tmc_obs = np.zeros((4,3))
            tmc_obs[:, 0] = np.concatenate([navigator_dists[0:ind], navigator_dists[ind + 1:]]) / self.comm_radius
            tmc_obs[:, 1] = np.cos(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))
            tmc_obs[:, 2] = np.sin(np.concatenate([navigator_bearings[0:ind], navigator_bearings[ind + 1:]]))

            # 对 int_points_obs 进行排序和筛选
            sorted_indices_int = np.argsort(int_points_obs[:, 0])  # 根据第一列（距离）排序并获取排序后的索引
            # selected_int_points_obs = int_points_obs[sorted_indices_int[:20]]  # 选择距离最近的前20个点的信息
            selected_int_points_obs = int_points_obs[sorted_indices_int[:10]]  # 选择距离最近的前20个点的信息

            # 对 tmc_obs 进行排序和筛选
            sorted_indices_tmc = np.argsort(tmc_obs[:, 0])  # 根据第一列（距离）排序并获取排序后的索引
            # selected_tmc_obs = tmc_obs[sorted_indices_tmc[:3]]  # 选择距离最近的前3个点的信息
            selected_tmc_obs = tmc_obs[sorted_indices_tmc[:1]]  # 选择距离最近的前3个点的信息

            obs = np.hstack([selected_tmc_obs.flatten(), selected_int_points_obs.flatten(),local_obs.flatten(), equ_obs.flatten()])
            # obs = np.hstack([tmc_obs.flatten(), int_points_obs.flatten(),local_obs.flatten(), equ_obs.flatten()])

        # 下边是采用ctde之后用到的全部观测信息
        elif self.obs_mode == 'local_obs_mode':
            if self.obs_radius2 > self.world_size:
                dist_to_int_points = int_points_dists / self.obs_radius2
                angle_to_int_points = [np.cos(int_points_bearings), np.sin(int_points_bearings)]
                local_obs = np.zeros(self.dim_local_o)
            dist_to_int_points = int_points_dists / self.obs_radius2
            ...
            tmc_distances = np.concatenate([navigator_dists[0:ind], navigator_dists[ind + 1:]]) / self.comm_radius
            ...
            obs = np.hstack([tmc_obs.flatten(), int_points_obs.flatten(),local_obs.flatten(), equ_obs.flatten(),adjacency_vector])
