...
from amb.envs.dexhands.DexterousHands.bidexhands.utils.process_marl import (
    get_AgentIndex,
)
from amb.envs.dexhands.DexterousHands.bidexhands.utils.parse_task import (
    parse_task,
)
...
class DexHandsEnv:
    ...
    def get_env(self, env_args):
        ...
        # override cfg["env"] with env_args["env_args"] one by one
        if "override_args" in env_args and env_args["override_args"]:
            for key, value in env_args["override_args"].items():
                if key in cfg["env"]:
                    print(f"Overriding {key}: {cfg['env'][key]} --> {value}")
                    cfg["env"][key] = value
