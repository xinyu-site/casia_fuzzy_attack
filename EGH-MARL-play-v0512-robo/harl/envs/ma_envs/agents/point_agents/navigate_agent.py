...
class NavigateAgent(Agent):
    def __init__(self, experiment):
        ...
        if self.obs_mode == '2d_rbf':
            ...
            self.dim_local_o = 3*int(not self.torus) + int(self.obs_radius2 <= 100)
            self.dim_flat_o = np.prod(self.dim_int_points_o) + self.dim_local_o
            self.dim_mean_embs = None
            self._dim_o = np.prod(self.dim_rec_o) + np.prod(self.dim_int_points_o) + self.dim_local_o
        # elif self.obs_mode == 'xing_acc':
        #     self.dim_equ_o = 4
        ...
        #     self.dim_local_o = 3 * int(not self.torus)
        #     self.dim_equ_o = 4
        #     self.dim_int_points_o = (self.int_points_num, 3)
        #     self.dim_flat_o = np.prod(self.dim_int_points_o) + self.dim_local_o
        #     self.dim_mean_embs = None
        ...
        #     self.dim_local_o = 3 * int(not self.torus)
        #     self.dim_equ_o = 4
        #     self.dim_int_points_o = (self.nearest_int_num, 3)
        #     self.dim_flat_o = np.prod(self.dim_int_points_o) + self.dim_local_o
        #     self.dim_mean_embs = None
        #     self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + np.prod(self.dim_int_points_o) + self.dim_equ_o
    ...
    def get_observation(self, dm, my_orientation, their_orientation, feat, vels):
        ...
        if self.obs_mode == 'fix':
            ...
            if self.local_mode:
                ...
                obs = np.hstack([eghn_obs.flatten(), return_int_points_obs.flatten(), equ_obs.flatten(), adjacency_vector])
            else:
            ...
            if self.local_mode:
                ...
                obs = np.hstack([equ_obs.flatten(), rec_obs.flatten(), int_points_obs.flatten(), local_obs.flatten(), adjacency_vector])
            else:
                obs = np.hstack([equ_obs.flatten(), rec_obs.flatten(), int_points_obs.flatten(), local_obs.flatten()])
