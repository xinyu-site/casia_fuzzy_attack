...
dynamics = ['point', 'unicycle', 'box2d', 'direct', 'direct_acc', 'direct_acc2', 'unicycle_acc', 'direct_nav2', 'direct_sb' , 'direct_asym']
...
class World(object):
    def __init__(self, world_size, torus, agent_dynamic , env_num1 = 0.0 , env_num2 = 0.0):
        
        self.env_num1 = env_num1
        self.env_num2 = env_num2
    ...
    def reset(self):
        ...
        if not hasattr(self, "A_env"):
            # choose an axis angle not aligned with D4 (avoid multiples of pi/4)
            # psi = np.random.uniform(0, 2*np.pi)
            psi = np.pi / 4
            psi += np.pi/16  # small offset to avoid accidental alignment
            ...
            # anisotropy strength
            eps = self.env_num1
            S = np.diag([1.0 + eps, 1.0 - eps])  # not proportional to I => breaks D4

            self.A_env = R @ S @ R.T  # (2,2), fixed in world coordinates

    def step(self):
        #print('env_num1:', self.env_num1, 'env_num2:', self.env_num2)
        ...
        if self.agent_dynamic == 'direct_acc':
            ...
            for i, agent in enumerate(self.policy_agents):
                actions[i, 0] = agent.action.u[0]
                actions[i, 1] = agent.action.u[1]
            scaled_actions = action * agent.max_lin_acceleration
            velocities = np.vstack([agent.state.p_vel for agent in self.policy_agents])
            damping = np.vstack([np.hstack([agent.lin_damping, agent.lin_damping]) for agent in self.policy_agents])  # 使用线性阻尼，假设对x和y方向相同
            ...
            for i in range(self.action_repeat):
                ...
                # scale_factor = agent.max_lin_velocity / (current_norm + 1e-8)
                # 如果范数超过最大值，则对速度进行缩放
                velocities[:, 0] = np.where(np.abs(velocities[:, 0]) <= agent.max_lin_velocity, velocities[:, 0],
                                            np.sign(velocities[:, 0]) * agent.max_lin_velocity)
                velocities[:, 1] = np.where(np.abs(velocities[:, 1]) <= agent.max_lin_velocity, velocities[:, 1],
                                            np.sign(velocities[:, 1]) * agent.max_lin_velocity)
                next_coord = self.agent_states[:, 0:2] + velocities * self.dt 
                if self.torus:
                    next_coord = np.where(next_coord < 0, next_coord + self.world_size, next_coord)
                    next_coord = np.where(next_coord > self.world_size, next_coord - self.world_size, next_coord)
            ...
            for i in range(self.action_repeat):
                ...
                agent_states_next = np.concatenate([next_coord, next_angles], axis=1)
                self.agent_states = agent_states_next
            # print("self.agent_states:", self.agent_states)
            # print("agent_states_next:", agent_states_next)
        ...
        elif self.agent_dynamic == 'direct_nav2':
            # 此动力学仅适用于带障碍物版本的导航
            obstacles = self.landmark_states[self.int_points_num:]
            velocities = np.zeros([self.nr_agents, 2]) 
            for i, agent in enumerate(self.policy_agents):
                velocities[i, 0] = agent.action.u[0] * agent.max_lin_velocity
                velocities[i, 1] = agent.action.u[1] * agent.max_lin_velocity
            for i in range(self.action_repeat):
                ...
                # 检测与障碍物的冲突并调整位置和速度
                for j, next_pos in enumerate(next_coord):
                    for obstacle in obstacles:
                        distance = np.linalg.norm(next_pos - obstacle[:2])
                        if distance <= self.obstacle_radius:
                            # 计算从智能体到障碍物中心的单位向量
                            direction = (next_pos - obstacle[:2]) / distance
                            # 更新智能体位置到障碍物边缘
                            next_coord[j] = obstacle[:2] + direction * self.obstacle_radius + 0.05
                            # 将速度设置为0
                            velocities[j] = np.zeros(2)
                            next_angles[j] = np.zeros((1, ))
                if self.torus:
                    next_coord = np.where(next_coord < 0, next_coord + self.world_size, next_coord)
                    next_coord = np.where(next_coord > self.world_size, next_coord - self.world_size, next_coord)
                else:
        ...
        elif self.agent_dynamic == 'direct_sb':
            # direct dynamics with symmetry-breaking noise
            velocities = np.zeros([self.nr_agents, 2])
            for i, agent in enumerate(self.policy_agents):
                velocities[i, 0] = agent.action.u[0] * agent.max_lin_velocity
                velocities[i, 1] = agent.action.u[1] * agent.max_lin_velocity
            ...
            for _ in range(self.action_repeat):
                noisy_vel = velocities
                
                noisy_vel = noisy_vel + np.random.normal(self.env_num1, self.env_num2, size=noisy_vel.shape)

                current_norm = np.linalg.norm(noisy_vel, axis=1)
                scale_factor = agent.max_lin_velocity / (current_norm + 1e-8)
                noisy_vel[:, 0] = np.where(current_norm <= agent.max_lin_velocity, noisy_vel[:, 0],
                                            noisy_vel[:, 0] * scale_factor)
                noisy_vel[:, 1] = np.where(current_norm <= agent.max_lin_velocity, noisy_vel[:, 1],
                                            noisy_vel[:, 1] * scale_factor)

                next_coord = self.agent_states[:, 0:2] + noisy_vel * self.dt
                if self.torus:
                    next_coord = np.where(next_coord < 0, next_coord + self.world_size, next_coord)
                    next_coord = np.where(next_coord > self.world_size, next_coord - self.world_size, next_coord)
                else:
                ...
                next_angles = np.arctan2(noisy_vel[:, 1], noisy_vel[:, 0])
                next_angles = next_angles[:, np.newaxis]
                agent_states_next = np.concatenate([next_coord, next_angles], axis=1)
                self.agent_states = agent_states_next

        elif self.agent_dynamic == 'direct_asym':
            # direct dynamics
            velocities = np.zeros((self.nr_agents, 2), dtype=np.float32)
            #print('yes!')
            vmax = np.array([a.max_lin_velocity for a in self.policy_agents], dtype=np.float32)  # (N,)

            # map action -> commanded velocity
            for i, agent in enumerate(self.policy_agents):
                velocities[i, 0] = agent.action.u[0] * agent.max_lin_velocity
                velocities[i, 1] = agent.action.u[1] * agent.max_lin_velocity

            for _ in range(self.action_repeat):
                # 1) clip by norm (rotation-invariant clipping)
                current_norm = np.linalg.norm(velocities, axis=1)  # (N,)
                scale_factor = vmax / (current_norm + 1e-8)
                cond = current_norm <= vmax
                velocities = np.where(cond[:, None], velocities, velocities * scale_factor[:, None])

                # 2) apply environment anisotropy (break D4, keep translation)
                #    A_env should be fixed and not proportional to I
                velocities = velocities @ self.A_env.T

                # 3) clip again to enforce vmax after anisotropy
                current_norm = np.linalg.norm(velocities, axis=1)
                scale_factor = vmax / (current_norm + 1e-8)
                cond = current_norm <= vmax
                velocities = np.where(cond[:, None], velocities, velocities * scale_factor[:, None])
                ...
                # 5) orientation follows executed velocity
                next_angles = np.arctan2(velocities[:, 1], velocities[:, 0])[:, None]
                agent_states_next = np.concatenate([next_coord, next_angles], axis=1)
                self.agent_states = agent_states_next
            ...
            for i, agent in enumerate(self.policy_agents):
                ...
                clipped_vel_norm = np.linalg.norm(velocities[i, :], keepdims=True)
                agent.energy_consumption = agent.hover_energy_cost + (clipped_vel_norm / agent.max_lin_velocity * agent.move_energy_cost)
