...
from harl.envs.ma_envs.agents.point_agents.rendezvous_agent import PointAgent
...
class RendezvousEnv(gym.Env, EzPickle):
    ...
    def ue_set(self, status):
        ...
        agent_states = np.zeros((self.nr_agents, 3))
        agent_states[:, 0:2] = states[:, 0:2]
        agent_states[:, 2:3] = states[:, 2:3]
        ...
        for i, agent in enumerate(self.world.policy_agents):
            agent.set_ue_vel(states[i, 3:])
        nr_agents_sensed = np.sum((0 < self.world.distance_matrix) &
                                  (self.world.distance_matrix < self.comm_radius), axis=1)
        obs = []
    ...
    def step(self, actions):
        self.timestep += 1
        #print(f'shape of actions: {actions.shape}  shape of action space: {self.agents[0].action_space.shape}')
