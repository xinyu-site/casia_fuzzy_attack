...
class DeterministicEghnPolicy(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self.pi = EGHN(in_node_nf=self.inv_nf, in_edge_nf=1, hidden_nf=hidden_sizes[-1], device=device,
    ...
    def forward(self, obs):
        ...
        loc_pred, vel_pred, _ = self.pi(loc, inv_fea, edges, edge_attr, local_edge_index, local_edge_fea, 
                                        n_node=self.n_nodes, v=vel, node_mask=None, node_nums=None, flag=False)
        self.lp_loss = self.pi.cut_loss
        x = vel_pred
