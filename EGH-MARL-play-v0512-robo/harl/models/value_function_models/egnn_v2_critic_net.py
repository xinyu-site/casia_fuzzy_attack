...
class EgnnV2CriticNet(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        """
        super(EgnnV2CriticNet, self).__init__()
        ...
        if self.use_history:
            ...
            # 仅等变数据采用时序矩阵加权
            # self.history_weight = PartLinearLayer(self.windows_size, self.equ_nf, self.equ_nf + self.inv_nf_old)
            # 所有数据均采用时序加权
            # self.history_weight = AllLinearLayer(self.windows_size, self.equ_nf + self.inv_nf_old)
            # 仅使用最后一个时刻的数据，检查数据正确性
            # self.history_weight = StaticWeightLinearLayer(input_features=self.windows_size, output_features=self.c)
            
        self.critic_fc1 = nn.Linear(1 + self.hidden_nf[0], 1)
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        if self.use_history:
            cent_obs = np.reshape(cent_obs, (-1, self.windows_size, self.n_nodes, (self.local_tool.equ_nf + self.local_tool.inv_nf_old)))
            cent_obs = check(cent_obs).to(**self.tpdv)
            cent_obs = cent_obs.permute(0, 2, 3, 1)
            cent_obs = cent_obs.reshape(-1, (self.local_tool.inv_nf_old + self.local_tool.equ_nf), self.windows_size)
            cent_obs = self.history_weight(cent_obs)
        else:
        ...
        loc_pred, vel_pred, h = self.egnn_model(loc, h, edges, edge_attr, v=vel)
