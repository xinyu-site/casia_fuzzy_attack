...
class GATCriticNet(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        """
        super(GATCriticNet, self).__init__()
        ...
        self.obs_shape = int(cent_obs_shape[0] / self.n_nodes)
       ...
       # 局部信息处理
        ...
        self.local_tool = local_tool(self, args=args, net_type="gat", id="critic")
        ...
        for i in range(0, self.n_layers):
            if i == 0:
                self.add_module("critic_gat_%d" % i, GATConv(int(self.hidden_sizes[0]), int(self.hidden_sizes[0]), heads=self.head_num))
            else:
                self.add_module("critic_gat_%d" % i, GATConv(int(self.hidden_sizes[0] * self.head_num), int(self.hidden_sizes[0]), heads=self.head_num))
        self.critic_fc1 = nn.Linear(self.hidden_sizes[0] * self.head_num, 1)
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        for j in range(0, self.n_layers):
            x_res = x  # 保留原始输入
            x = self._modules["critic_gat_%d" % j](x, edges)
            if self.use_res:
                if not (self.head_num > 1 and j == 0):
                    x = x + x_res  # critic应用残差连接
            x = torch.tanh(x)
