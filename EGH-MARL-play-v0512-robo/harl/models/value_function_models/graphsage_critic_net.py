...
class GraphSAGECriticNet(nn.Module):
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        """
        super(GraphSAGECriticNet, self).__init__()
        ...
        self.local_tool = local_tool(self, args=args, net_type="graphsage", id="critic")
        ...
        for i in range(0, self.n_layers):
            self.add_module("critic_graphsage_%d" % i, SAGEConv(int(self.hidden_sizes[0]), int(self.hidden_sizes[0])))            
    ...
    def forward(self, cent_obs, rnn_states, masks):
        ...
        for j in range(0, self.n_layers):
            x_res = x  # 保留原始输入
            x = self._modules["critic_graphsage_%d" % j](x, edges)
            if self.use_res:
                x = x + x_res  # 应用残差连接
            x = torch.tanh(x)
