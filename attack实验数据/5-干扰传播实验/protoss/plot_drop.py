...
def plot_drop_rates(df, algorithms, attack_levels):
    ...
    for intensity in attack_levels[1:]:  # 跳过基线
        ...
        for algo in algorithms:
            ...
            baseline = algo_data[algo_data['noise_intensity'] == baseline_intensity]['avg_reward'].mean()
            attacked = algo_data[algo_data['noise_intensity'] == intensity]['avg_reward'].mean()
...
def main():
    ...
    # 读取目录下所有eval_result_*_avg.txt文件
    all_data = []
    current_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 获取所有eval_result_*_avg.txt文件
    pattern = os.path.join(current_dir, 'orin_eval_result_*.txt')
    ...
    for algorithm in algorithms:
        ...
        for intensity in attack_levels:
            ...
            if len(intensity_data) > 0:
                reward = intensity_data['avg_reward'].mean()
    ...
    for algorithm in algorithms:
        ...
        for intensity in attack_levels:
            reward = algo_data[algo_data['noise_intensity'] == intensity]['avg_reward'].mean()
            print(f"   • 攻击强度 {intensity_labels[intensity]}: 平均奖励 = {reward:.4f}")
        
        baseline_reward = algo_data[algo_data['noise_intensity'] == baseline_intensity]['avg_reward'].mean()
        print(f"   📈 性能变化:")
        for intensity in attack_levels[1:]:
            attacked_reward = algo_data[algo_data['noise_intensity'] == intensity]['avg_reward'].mean()
            drop_rate = calculate_drop_rate(baseline_reward, attacked_reward)
            if drop_rate > 0:
