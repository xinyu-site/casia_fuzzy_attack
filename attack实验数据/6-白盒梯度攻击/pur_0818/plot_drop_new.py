...
# 计算攻击后保留的性能百分比
def calculate_retention_rate(baseline_reward, attacked_reward):
    if baseline_reward == 0:
        return 0
    return (attacked_reward / baseline_reward) * 100
...
def plot_drop_rates(df, algorithms, attack_levels):
    ...
    for intensity in attack_levels[1:]:  # 跳过基线
        ...
        for algo in algorithms:
            ...
            retention_rate = calculate_retention_rate(baseline, attacked)
            retention_rates.append(retention_rate)
        retention_rates_data[f'{intensity}'] = retention_rates
    
    # 创建图形
    fig, ax = plt.subplots(figsize=(12, 7))
    
    x = np.arange(len(algorithms))
    width = 0.8 / len(retention_rates_data)
    
    # 绘制分组柱状图
    for i, (attack_name, retention_rates) in enumerate(retention_rates_data.items()):
        offset = (i - len(retention_rates_data)/2 + 0.5) * width
        bars = ax.bar(x + offset, retention_rates, width, label=attack_name,
                     color=colors[i % len(colors)], alpha=0.8, edgecolor='black', linewidth=0.5)
        
        # 在柱子上添加数值标签
        for bar, retention_rate in zip(bars, retention_rates):
            height = bar.get_height()
            ax.text(bar.get_x() + bar.get_width()/2, height,
                   f'{retention_rate:.1f}%', ha='center', va='bottom',
                   fontsize=9, fontweight='bold')
    
    # 设置图表属性
    ax.set_xlabel('Algorithm', fontsize=12, fontweight='bold')
    ax.set_ylabel('Performance Retained (%)', fontsize=12, fontweight='bold')
    ax.set_title('Performance Retained Under Different Attack Intensities',
    ...
    # 设置y轴范围（留出一些空间给数值标签）
    all_retention_rates = [rate for rates in retention_rates_data.values() for rate in rates]
    data_range = max(all_retention_rates) - min(all_retention_rates)
    padding = max(data_range * 0.12, 5)
    y_min = min(all_retention_rates) - padding
    y_max = max(max(all_retention_rates), 100) + padding
    ax.set_ylim(y_min, y_max)
    
    # 添加水平参考线
    ax.axhline(y=100, color='red', linestyle='--', linewidth=1, alpha=0.6,
               label='Baseline (100%)')
    ax.legend(loc='upper right', fontsize=10, framealpha=0.9)
    
    plt.tight_layout()
    plt.savefig('performance_retention_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("✅ 性能保留率对比图已保存: performance_retention_comparison.png")

def main():
    # 读取数据
    data = parse_data('eval_result_pursuit.txt')
    ...
    print("   • performance_retention_comparison.png - 性能保留率对比柱状图")
    print("="*120)
