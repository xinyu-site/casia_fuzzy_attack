...
class OffPolicyBaseRunner:
    ...
    def run(self):
        ...
        for episode in range(1, episodes + 1):
            ...
            for t in range(self.algo_args["train"]["episode_length"]):
                ...
                self.logger.per_step_offpolicy(data)
                self.insert(data)
                obs = new_obs
                ...
                if step % self.algo_args["train"]["train_interval"] == 0:
                    ...
                    for _ in range(update_num):
                        # self.train()
                        actor_train_info, critic_train_info = self.train()
                        self.logger.log_train_offpolicy(actor_train_info, critic_train_info, self.total_it)
            if episode % self.algo_args["train"]["log_interval"] == 0:
                r = self.logger.episode_log_offpolicy()
                # if r != None and r >= max_r:
                #     max_r = r
                #     self.save()
                self.save()
            if episode % self.algo_args["train"]["eval_interval"] == 0:
                if self.algo_args["eval"]["use_eval"]:
                    # self.prep_rollout()
                    self.eval(step * self.algo_args["train"]["n_rollout_threads"])
                # self.save()
    ...
    def insert(self, data):
        ...
        if self.algo_args["train"]["n_rollout_threads"] > 1:
                ...
                next_share_obs = next_share_obs.squeeze()
                t_rewards = np.reshape(rewards[:, 0], (self.algo_args["train"]["n_rollout_threads"],1))
        if self.state_type == "EP":
            data = (
    ...
    def get_actions(self, obs, available_actions=None, add_random=True):
        ...
        if self.args["algo"] == "hasac":
            ...
            if self.flag:
                ...
                actions = self.actor[0].get_actions(np.concatenate(obs), add_random)
            ...
            if eval_episode >= self.algo_args["eval"]["eval_episodes"]:
                ...
                if "smac" in self.args["env"]:
                    ...
                    self.writter.add_scalar(
                        "eval_win_rate", eval_battles_won / eval_episode, step
                    )
    ...
    def restore(self):
        """Restore the model"""
        if self.flag:
            self.actor[0].restore(self.algo_args["train"]["model_dir"], 0)
        else:
            for agent_id in range(self.num_agents):
    ...
    def save(self):
        """Save the model"""
        if self.flag:
            self.actor[0].save(self.save_dir, 0)
        else:
            for agent_id in range(self.num_agents):
