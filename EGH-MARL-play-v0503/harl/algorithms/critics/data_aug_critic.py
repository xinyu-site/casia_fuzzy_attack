...
class VCritic_data_aug:
    ...
    def __init__(self, args, cent_obs_space, device=torch.device("cpu")):
        ...
        # elif "eghn_critic" in args and args["eghn_critic"]:
        #     self.critic = EghnCriticNet(args, self.share_obs_space, self.device)
        # else:
    ...
    def train(self, critic_buffer, critic_buffer_90, critic_buffer_180, critic_buffer_270,value_normalizer=None):
        ...
        for _ in range(self.critic_epoch):
            ...
            if self.use_recurrent_policy:
                ...
                data_generator1 = critic_buffer.feed_forward_generator_critic(
                    self.critic_num_mini_batch
                )
                data_generators.append(data_generator1)

                # data_generator2 = critic_buffer_90.feed_forward_generator_critic(
                #     self.critic_num_mini_batch
                # )
                # data_generators.append(data_generator2)

                # data_generator3 = critic_buffer_180.feed_forward_generator_critic(
                #     self.critic_num_mini_batch
                # )
                # data_generators.append(data_generator3)

                data_generator = critic_buffer_270.feed_forward_generator_critic(
                    self.critic_num_mini_batch
                )
            ...
            value_loss, critic_grad_norm = self.update(
                        batches, value_normalizer=value_normalizer
                        )
            train_info["value_loss"] += value_loss.item()
            train_info["critic_grad_norm"] += critic_grad_norm
