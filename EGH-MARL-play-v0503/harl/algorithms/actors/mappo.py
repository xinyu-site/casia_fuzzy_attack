...
class MAPPO(OnPolicyBase):
    ...
    def update(self, sample):
        ...
        # for i, layer in enumerate(self.actor.base.mlp.fc):
        #     if hasattr(layer, 'weight') and torch.isnan(layer.weight).any():
        #         print(f"Layer {i} ('{layer.__class__.__name__}') contains NaN in weights.")
        #     if hasattr(layer, 'bias') and layer.bias is not None and torch.isnan(layer.bias).any():
        #         print(f"Layer {i} ('{layer.__class__.__name__}') contains NaN in bias.")
