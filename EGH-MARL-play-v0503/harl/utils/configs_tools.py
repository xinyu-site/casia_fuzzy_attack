...
def get_task_name(env, env_args):
    ...
    if env == "smac":
        ...
        obs_mode = env_args["obs_mode"].replace("_", "")
        dynamics = env_args["dynamics"].replace("_", "")
        task = f"{obs_mode}-{dynamics}"
    elif env == "pursuit":
        obs_mode = env_args["obs_mode"].replace("_", "")
        dynamics = env_args["dynamics"].replace("_", "")
        evader_policy = env_args["evader_policy"]
        task = f"{obs_mode}-{dynamics}-{evader_policy}"
    elif env == "navigation":
        obs_mode = env_args["obs_mode"].replace("_", "")
        dynamics = env_args["dynamics"].replace("_", "")
        task = f"{obs_mode}-{dynamics}"
