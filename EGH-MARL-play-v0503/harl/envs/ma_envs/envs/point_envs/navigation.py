...
from harl.envs.ma_envs.agents.point_agents.navigate_agent import NavigateAgent
...
class NavigationEnv_ori(gym.Env, EzPickle):
    ...
    def ue_set(self, status):
        ...
        int_points = status[0][self.n_agents:]
        
        if self.use_history:
            self.obs_his.clear_obs()
        self.timestep = 0
        assert self.nr_agents == len(states)
        # states:0-1:位置，2方位, 3-4速度
        ...
        navigators[:, 0:2] = states[:, 0:2]
        navigators[:, 2:3] = states[:, 2:3]
        
        self.world.agent_states = navigators
        self.world.landmark_states = int_points[:, 0:2]
        self.world.reset()
        self.distribute_target()
        feats = [p.graph_feature for p in self.agents]
        
        for i, agent in enumerate(self.world.agents):
            agent.state.p_vel = states[i, 3:]
        ...
        self.obs_history_li.append(s_obs)
        if len(self.obs_history_li) > 3:
            del self.obs_history_li[:-2]
    ...
    def reset(self):
        ...
        navigators = np.random.rand(self.nr_agents, 3)
        # 便于可视化，固定初始化区域
        # navigators[:, 0:2] = 15 * navigators[:, 0:2]
        # navigators[0:3, 0:2] = navigators[0:3, 0:2] + [40, 45]
        # navigators[3:6, 0:2] = navigators[3:6, 0:2] + [35, 40]
        # navigators[6:, 0:2] = navigators[6:, 0:2] + [30, 35]
        # 固定初始化目标点
        # int_points = np.array([
        #     [10, 55],
        #     [65, 10],
    ...
    def step(self, actions):
        ...
        self.obs_history_li.append(next_obs)
        if len(self.obs_history_li) > 3:
            del self.obs_history_li[:-2]

        rewards = self.get_reward_old(actions)
    ...
    def get_reward(self, actions):  
        ...
        for i in range(self.n_agents):
            r[i] = (self.obs_history_li[-2][i][(self.n_agents - 1) * 3] - self.obs_history_li[-1][i][(self.n_agents - 1) * 3]) * 100 * 141.5
    ...
    def get_reward_old(self, actions):  
        ...
        agent_dis = agent_dis[:, :-self.int_points_num]
        angle = self.world.angle_matrix[:, -self.int_points_num:]
        # angle = angle[:, -self.int_points_num:]
        r = np.zeros((self.nr_agents,1))
        # energy_rate = 0.1
        move_rew_rate = 1.0
        move_angle_rate = 0.002
        # slow_vel_rew_rate = 1.0

        collide_rew_rate1 = -4.0
        collide_rew_rate2 = -20.0
        ...
        for j in range(self.n_agents):
            for i in range(self.int_points_num):
                if dis[j][i] <= self.bear_dis:
                    if i == self.target_list[j]:
                        # reach_target_set.add(j)
                        # r[j] += 1/(vec_norm_list[j] + 0.2) * slow_vel_rew_rate
                        r[j] += get_target_rew
                        break
        ...
        #             else:
        #                 collide_dis = 1/(agent_dis[j][i] + self.epislon) ** 2 + 3.0
        #                 r[j] += collide_dis * collide_rew_rate2
        # r = np.ones((self.nr_agents, 1)) * r.mean()
        return r
