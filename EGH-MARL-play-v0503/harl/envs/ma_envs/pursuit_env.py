...
class PursuitEnv:
    def __init__(self, args):
        self.args = args
        self.env = PursuitEvasionEnv(local_mode=self.args["local_mode"],
    ...
    def make_ani(self, trajectories):
        ...
        # 更新函数，每次调用都会更新图表
        def update(frame):
            nonlocal sc, arrows, sc1, trail_scatters, trail_scatters1
            ...
            for sc_trail in trail_scatters:
                sc_trail.remove()
            for sc1_trail in trail_scatters1:
                sc1_trail.remove()
            trail_scatters = []
            trail_scatters1 = []
            ...
            for i in range(window, -1, -1):
                ...
                px = pos_x_list[idx]
                py = pos_y_list[idx]
                px1 = eva_pos_x_list[idx]
                py1 = eva_pos_y_list[idx]
                ...
                sc1 = ax.scatter(px1, py1, c='black',marker='o', alpha=alpha*0.8, s=size*2.5)
                trail_scatters.append(sc_trail)
                trail_scatters1.append(sc1)
            # 绘制散点图，根据分组数量动态更新色系
            # if sc is not None:
            #     sc.remove()
            # if len(pooling_plan) > 0:
            ...
            print(show_index)
            plt.savefig(f'./hie_fig/pursuit/{show_index:03d}.pdf', bbox_inches='tight', pad_inches=0.02)
            # show_index += 1
