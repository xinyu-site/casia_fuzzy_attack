...
class PointAgent(Agent):
    def __init__(self, experiment):
        ...
        #     self.dim_flat_o = np.prod(self.dim_evader_o) + self.dim_local_o
        #     self.dim_mean_embs = None
        #     self.dim_adjacency_o = self.n_agents
        #     self._dim_o = np.prod(self.dim_rec_o) + self.dim_local_o + self.dim_equ_o + np.prod(self.dim_evader_o) + self.dim_adjacency_o
        # elif self.obs_mode == 'temp_obs_mode':
        #     self.dim_rec_o = (self.nearest_num, 3)
        #     self.dim_local_o = 3 * int(not self.torus)
        #     self.dim_equ_o = 4
        #     self.dim_int_points_o = (self.n_evaders, 3)
    ...
    def get_observation(self, dm, my_orientation, their_orientation, feat, vels):
        ...
        if self.obs_mode == 'fix':
            ...
            if self.local_mode:
                local_distances = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.comm_radius
                ...
                obs = np.hstack([eghn_obs.flatten(), evader_obs.flatten(), equ_obs.flatten(), adjacency_vector])
            else:
            ...
            dis_to_other = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]])
            orientation = np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]])
            ...
            if self.local_mode:
                ...
                # used for EGNN, EGHN
                adjacency_vector = np.where(pursuer_dists <= self.obs_radius, 1, -1)
                adjacency_vector[ind] = -1
                obs = np.hstack([equ_obs.flatten(), rec_obs.flatten(), evader_obs.flatten(), local_obs.flatten(), adjacency_vector])
            else:
                obs = np.hstack([equ_obs.flatten(), rec_obs.flatten(), evader_obs.flatten(), local_obs.flatten()])
        # elif self.obs_mode == 'eghn':
        #     dist_to_evader = evader_dists / self.obs_radius2
        #     angle_to_evader = [np.cos(evader_bearings), np.sin(evader_bearings)]
        ...
        #     rec_obs = np.zeros(self.dim_rec_o)
        #     # tmc_obs[:, 0] = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.comm_radius
        #     rec_obs[:, 0] = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.distance_threshold
        #     rec_obs[:, 1] = np.cos(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))
        #     rec_obs[:, 2] = np.sin(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))

        #     # tmc_distances = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.comm_radius
        #     adjacency_vector = np.ones(self.n_agents) * -1  # 初始化为-1，表示没有连接
        #     # adjusted_distances = np.insert(tmc_distances, ind, -1)  # 在当前智能体位置插入一个占位值
        #     for i, distance in enumerate(pursuer_dists):
        #         if i != ind and distance < self.distance_threshold:
        #             adjacency_vector[i] = 1  # 如果智能体间距离小于阈值，则设置为1
        #             rec_obs[i, 0] = 1.
        #             rec_obs[i, 1] = 0
        #             rec_obs[i, 2] = 0

        #     obs = np.hstack([rec_obs.flatten(), evader_obs.flatten(), local_obs.flatten(), equ_obs.flatten(), adjacency_vector])
        # elif self.obs_mode == 'temp_obs_mode':
        #     dist_to_int_points = evader_dists / self.obs_radius
        #     angle_to_int_points = [np.cos(evader_bearings), np.sin(evader_bearings)]
        #     evader_obs = np.zeros((self.n_evaders, 3))
        #     evader_obs[:, 0] = dist_to_int_points
        #     evader_obs[:, 1] = angle_to_int_points[0]
        #     evader_obs[:, 2] = angle_to_int_points[1]
        ...
        #     fix_obs = np.zeros((self.n_agents - 1,3))
        #     fix_obs[:, 0] = np.concatenate([pursuer_dists[0:ind], pursuer_dists[ind + 1:]]) / self.comm_radius
        #     fix_obs[:, 1] = np.cos(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))
        #     fix_obs[:, 2] = np.sin(np.concatenate([pursuer_bearings[0:ind], pursuer_bearings[ind + 1:]]))

        #     # 对 tmc_obs 进行排序和筛选
        #     sorted_indices_tmc = np.argsort(fix_obs[:, 0])  # 根据第一列（距离）排序并获取排序后的索引
        #     selected_fix_obs = fix_obs[sorted_indices_tmc[:self.nearest_num]]  # 选择距离最近的前5个点的信息

        #     obs = np.hstack([selected_fix_obs.flatten(), evader_obs.flatten(), local_obs.flatten(), equ_obs.flatten()])
        elif self.obs_mode == 'sum_obs':
            ...
            # neighbor obs
            evaders_in_range = (evader_dists < self.obs_radius2) & (0 < evader_dists)
            pursuers_in_range = (pursuer_dists < self.comm_radius) & (0 < pursuer_dists)
            nr_neighbors = np.sum(pursuers_in_range)
            ...
            in_range = (evader_dists <= self.obs_radius2) & (0 <= evader_dists)
            nr_neighboring_evaders = np.sum(in_range)
            dist_to_evader = evader_dists[in_range] / self.obs_radius2
            ...
            evader_in_range = (evader_dists < self.obs_radius2) & (0 < evader_dists)
            ...
            if self.torus is False:
                if np.any(self.state.p_pos <= 1) or np.any(self.state.p_pos >= 99):
                    ...
                    wall = 0
                local_obs[0] = wall
            evader_in_range = evader_dists < self.obs_radius2
