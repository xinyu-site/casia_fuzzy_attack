...
def main():
    ...
    # 自动加载训练时保存的配置文件（如果存在）
    if args["env"] == "smacv2":
        config_path = f'{args["results_dir"]}/{args["env"]}/{args["map_name"]}/{args["algo"]}/{args["exp_name"]}'
    elif args["env"] == "pursuit":
        config_path = f'{args["results_dir"]}/{args["env"]}/{env_args["obs_mode"].replace("_", "")}-{env_args["dynamics"].replace("_", "")}-tac/{args["algo"]}/{args["exp_name"]}'
    else:
        config_path = f'{args["results_dir"]}/{args["env"]}/{env_args["obs_mode"].replace("_", "")}-{env_args["dynamics"].replace("_", "")}/{args["algo"]}/{args["exp_name"]}'
    print(f'config_path: {config_path}')
    if os.path.exists(config_path):
        print('finding config file')
        sub_folders = [f for f in os.listdir(config_path) if os.path.isdir(os.path.join(config_path, f))]
        ...
        if matched_results:
            ...
            saved_config_path = os.path.join(config_path, selected_folder, "config.json")
            if os.path.exists(saved_config_path):
                print(f"Loading saved config from: {saved_config_path}")
                with open(saved_config_path, encoding="utf-8") as file:
                    saved_config = json.load(file)
                algo_args = saved_config["algo_args"]
                env_args = saved_config["env_args"]
    ...
    with open(f"eval_result_{args['env']}_single.txt", "a", encoding="utf-8") as f:
        f.write(result_line)
